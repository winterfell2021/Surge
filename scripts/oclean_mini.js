...
function grad(redId=2){
    return new Promise((resolve, reject) => {
        console.log(taskUrl(redId))
        $.get(taskUrl(redId), (err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (data['Status'] === 'OK') {
                        if(data['Code'] === 1){
                            ...
                            console.log(`抽奖失败，已抽奖`)
                            message += `【用户${$.index}】抽奖失败，已抽奖\n`
                        } else{
                            console.log(`抽奖失败，${$.toStr(data)}`)
...
function taskUrl(redId) {
    return {
        url: `https://mall.oclean.com/API/VshopProcess.ashx?action=GrabEveryDayPoint&redId=${redId}&clientType=5&client=5&openId=${cookie}`,
    }
}
