...
async function sign() {
    ...
    request({
      url: 'https://mxsa.mxbc.net/api/v1/customer/signin?' + getSign(),
      headers: headers()
    },async (err,resp,data)=>{
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code === 0)
                message += `【蜜雪冰城】账号${$.index} 签到成功，获得${data.data.ruleValuePoint}雪王币 \n`
                
            else
                message += `【蜜雪冰城】账号${$.index} 签到失败，${data.msg} \n`
          }
        }
      }
      catch (e) {
...
async function getLoginUrl() {
    return new Promise(resolve => {
    request({
      url: 'https://mxsa.mxbc.net/api/v1/duiba/getLoginUrl?' + getSign(),
      headers: headers()
    },async (err,resp,data)=>{
      try {
        if (err) {
...
async function getPoint() {
    return new Promise(resolve => {
    request({
      url: 'https://mxsa.mxbc.net/api/v1/customercoupons/total',
      method: 'POST',
      body: getSign({t: +new Date(),appId: "d82be6bbc1da11eb9dd000163e122ecb", "couponStatus": 0}, true),
      headers: headers()
    },async (err,resp,data)=>{
      try {
        if (err) {
...
function getSign(E = {t: +new Date(),appId: "d82be6bbc1da11eb9dd000163e122ecb"}, post = false){
    ...
    for(let o = 0; o < c.length; o++) {
                var a = c[o];
                if (E[a] || 0 === E[a]) {
                    var i = "object" === typeof E[a] ? JSON.stringify(E[a]) : E[a];
                    A += "".concat(o ? "&" : "").concat(a, "=").concat(i)
                } else "" !== E[a] && delete E[a]
    }
    n.updateString(A);
    let s = t.hextob64(n.sign()).replace(/\//g, "_").replace(/\+/g, "-")
    if (!post)
        return A + "&sign=" + s
    return JSON.stringify({...tmp,sign:s})
}
