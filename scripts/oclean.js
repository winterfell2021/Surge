...
function sign() {
    let options = taskUrl('', 'action=SignIn&SignInSource=Appshop&clientType=2')
    return new Promise((resolve, reject) => {
        $.post(options, (err, resp, data) => {
            try {
                if (err) {
...
function freeDraw(){
    let options = taskUrl('?action=ActivityDraw', 'ActivityId=9&clientType=2')
    return new Promise((resolve, reject) => {
        $.post(options, (err, resp, data) => {
            try {
                if (err) {
...
function taskUrl(function_url, data) {
    return {
        url: `https://mall.oclean.com/API/VshopProcess.ashx${function_url}`,
        body: data,
        headers:  {
